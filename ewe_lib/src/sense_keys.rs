...
use crate::wordnet::{Lexicon, PartOfSpeech, Sense, SenseId, Synset, SynsetId};
...
fn sense_for_entry_synset_id<'a, L: Lexicon>(
    wn: &'a L,
    ss_id: &SynsetId,
    lemma: &str,
) -> Result<Vec<Cow<'a, Sense>>> {
...
fn get_head_word<L: Lexicon>(wn: &L, ss: &Synset) -> Result<(String, String)> {
    ...
    let mut srs: Vec<&SynsetId> = ss
        .similar
        .iter()
        .filter(|target_id| {
            !target_id.as_str().starts_with("8") && !target_id.as_str().starts_with("9")
        })
        .collect();
    ...
    if srs.len() != 1 {
        ...
        match wn
            .members_by_id(tss)?
            .iter()
            .flat_map(|m| sense_for_entry_synset_id(wn, tss, m).unwrap())
            .next()
        {
            Some(s2) => {
                let (entry_id, _) = s2
                    .id
                    .as_str()
                    .split_at(s2.id.as_str().find("%").unwrap_or(0));
                Ok((
                    entry_id.to_string(),
                    format!("{:02}", extract_lex_id(&s2.id)),
                ))
            }
            None => Ok(("???".to_string(), "00".to_string())),
        }
    }
}

pub fn get_sense_key2<L: Lexicon>(
    wn: &L,
    lemma: &str,
        ...
        Some(synset) => {
            for entry in wn.entry_by_lemma(lemma)? {
                if entry.sense.iter().any(|sense| sense.synset == *synset_id) {
                    return Ok(Some(get_sense_key(
                        wn, lemma, sense_key, &synset, synset_id,
                    )?));
                }
            }
        }
...
pub fn get_sense_key<L: Lexicon>(
    wn: &L,
    lemma: &str,
    sense_key: Option<&SenseId>,
    synset: &Synset,
    ...
    let lex_filenum = wn
        .lex_name_for(synset_id)?
        .and_then(|lex_name| LEX_FILENUMS.get(lex_name.as_str()))
        .map(|x| *x)
        .unwrap_or(99);
...
mod tests {
    ...
    use crate::wordnet::{Entry, LexiconHashMapBackend, PosKey};
    ...
    fn test_sense_key_1() {
        ...
        lexicon
            ...
            .unwrap();
        assert_eq!(
            SenseId::new("foot%1:08:00::".to_owned()),
            get_sense_key(
                &lexicon,
                "foot",
                None,
    ...
    fn test_sense_key_2() {
        ...
        lexicon
            ...
            .unwrap();
        assert_eq!(
            SenseId::new("foot%1:08:02::".to_owned()),
            get_sense_key(
                &lexicon,
                "foot",
                None,
    ...
    fn test_sense_key_3() {
        ...
        lexicon
            ...
            .unwrap();
        assert_eq!(
            SenseId::new("scorching%5:00:01:hot:01".to_owned()),
            get_sense_key(
                &lexicon,
                "scorching",
    ...
    fn test_gen_id() {
        ...
        lexicon
            .insert_synset("adj.all".to_string(), ssid1.clone(), synset1.clone())
            .unwrap();
        lexicon
            .insert_synset("adj.all".to_string(), ssid2.clone(), synset2.clone())
            .unwrap();
